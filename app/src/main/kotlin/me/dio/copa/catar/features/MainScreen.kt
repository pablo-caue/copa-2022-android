...
@Composable
fun MatchInfo(matchDomain: MatchDomain, onNotificationClick :NotificationOnClick) {
    Card(
        shape = Shapes.large,
        ...
        Box {
            AsyncImage(
                model = matchDomain.stadium.image,
                contentDescription = null,
                contentScale = ContentScale.Crop,
                modifier = Modifier.height(160.dp)
            )
            Column(modifier = Modifier.padding(16.dp)) {
                Notification(matchDomain, onNotificationClick)
                Title(matchDomain)
                Teams(matchDomain)
            }
        }
